...
Cambios en v1.0.2
-Agregados atributos de simulación en config
-Arregrado error al crear el archivo "enviroment.txt"
-Eliminada libreria cvs no usada

cambios en v1.0.3
-Se agrego el parametro offset para temp y ldr en config
-Se crearon los datos temp_fix y ldr_fix aplicando offset a las medidas tomadas
...
# -------------------------
def simulate_reading(t_seconds,temp_initial,ldr_initial,temp_noicemax,ldr_noicemax):
    """
    ...
    Nota version 1.0.2
    - Se agregaron las variables siguientes para permiter ser cambiadas a traves de config:
        temp_initial
        ldr_initial
        temp_noisemax
        ldr_noisemax
    ...
    temp_base = temp_initial + 0.01 * (t_seconds / 1.0)  # deriva muy lenta
    temp_noise = random.gauss(0,temp_noicemax)            # ruido gaussiano
    ...
    ldr_base = ldr_initial + 50 * (0.5 * (1 + math.sin(t_seconds / 5.0)))
    ldr_noise = random.gauss(0,ldr_noicemax)
...
def acquire_data(mode="sim", port=None, baud=115200, duration_seconds=DEFAULT_DURATION, sample_interval_s=DEFAULT_INTERVAL,
                 temp_initial=22,ldr_initial=400, temp_noicemax=.2, ldr_noicemax=10):
    """
    ...
    try:
        while elapsed < duration_seconds:
            ...
            try:
                if mode == "sim":
                    temp, ldr = simulate_reading(elapsed,temp_initial,ldr_initial,temp_noicemax,ldr_noicemax)
                else:
...
# -------------------------
def save_csv(readings, csv_path,temp_offset,ldr_offset):
    """
    Guarda readings en CSV con encabezado.
    readings: lista de tuplas (timestamp_iso, temp, ldr)
    Implementa escritura atómica.
    
    v1.0.3 Se gregaron los parametros temp_fix y ldr_fix usando offset
    v1.0.4 Arreglado error de posición con los encabezados
    
    """
    header = ["timestamp", "temp_raw","temp_fix", "ldr_raw","ldr_fix"]
    lines = [",".join(header) + "\n"]
    for ts, temp, ldr in readings:
        temp_str = "" if (isinstance(temp, float) and temp != temp) else str(temp)
        temp_fix = str(temp - temp_offset)
        ldr_str = "" if ldr == -1 else str(ldr)
        ldr_fix = str(ldr - ldr_offset)
        lines.append(f"{ts},{temp_str},{temp_fix},{ldr_str},{ldr_fix}\n")
    text = "".join(lines)
...
def save_environment(env_path):
    ...
    nota de version 1.0.2 
    -Se elimino libs["matplotlib"] = plt.__version__
    ya que la version no se encuentra en el submodulo .pyplot usado
...
def build_argparser():
    ...
    p.add_argument("--temp_initial", type=float, default=22, help="Temperatura inicial de simulación")
    p.add_argument("--ldr_initial", type=float, default=400, help="ldr inicial de simulación")
    p.add_argument("--temp_noicemax",type=float, default=.2, help="rango maximo de ruido gaussiano para variación en sim. de temp.")
    p.add_argument("--ldr_noicemax",type=float, default=10, help="rango maximo de ruido gaussiano para variación en sim. de ldr")
    p.add_argument("--temp_offset",type=float, default=0,help="offset aplicado a las lecturas de Temp.")
    p.add_argument("--ldr_offset",type=float,default=0, help="offset aplicado a las lecturas de ldr")
    return p

def crearTemp_offset():
    """
    Nuevo en v1.0.4
    Función para crear un offset que se aplicara a las medidas de temperatura
    """
    I=1
    SumMuestras=0
    N=float(input("Ingrese el numero de muestras para temperatura: "))
    while I<N+1:
        Muestra=float(input("Ingrese la muestra: "))
        SumMuestras = SumMuestras + Muestra
        I=I+1
    temp_offset= (1/N)*(abs(SumMuestras))
    return temp_offset

def crearldr_offset():
    """
    Nuevo en v1.0.4
    Función para crear un offset que se aplicara a las medidas de ldr
    """
    I=1
    SumMuestras=0
    N=float(input("Ingrese el numero de muestras para ldr: "))
    while I<N+1:
        Muestra=float(input("Ingrese la muestra: "))
        SumMuestras = SumMuestras + Muestra
        I=I+1
    ldr_offset= (1/N)*(abs(SumMuestras))
    return ldr_offset
...
def main():
    ...
    config = {
        ...
        "Temp initial": args.temp_initial,
        "LDR initial": args.ldr_initial,
        "TempNoiceMax": args.temp_noicemax,
        "LDRNoiceMax": args.ldr_noicemax,
    ...
    ldr_offset=args.ldr_offset
    
    respuestaTemp=float(input("Ingrese 1 para crear un offset con medidas propias a las medidas de temperatura registradas, cualquier otro nuúmero para continuar y usar el default (0): "))
    if respuestaTemp==1:
        temp_offset=crearTemp_offset()
    respuestaLDR=float(input("Ingrese 1 para crear un offset con medidas propias a las medidas de ldr registradas, cualquier otro nuúmero para continuar y usar el default (0): "))
    if respuestaLDR==1:
        ldr_offset=crearldr_offset() 
    ...
    readings = acquire_data(mode=args.mode, port=args.port, baud=args.baud,
                            ...
                            temp_initial=args.temp_initial,ldr_initial=args.ldr_initial,
                            temp_noicemax=args.temp_noicemax,ldr_noicemax=args.ldr_noicemax)
    ...
    save_csv(readings, csv_path, temp_offset, ldr_offset)
